...
public:
    ...
    ParallelMatrix(int argc, char *argv[], int rows, int cols) : global_rows(rows), global_cols(cols)
    {
        MPI_Init(&argc, &argv);
        ...
        if (0 == rank)
        {
            A.resize(global_rows, global_cols);
            int nnz_per_row = global_cols < 10 ? global_cols : 10;          // 每列非零元素的数量，最多为10
            A.reserve(Eigen::VectorXi::Constant(global_cols, nnz_per_row)); // 每列预分配nnz_per_col个非零元素
            ...
            SparseMat A_transpose = A.transpose();
